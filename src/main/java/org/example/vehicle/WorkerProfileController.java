...
public class WorkerProfileController implements Initializable {
    ...
    @FXML
    TextField UserName, FullName, Email, Phone;
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                if(image == null){
                    ...
                    psInsertValue = connection.prepareStatement("UPDATE worker SET picture = ? WHERE wid = ?");
    ...
    public void updateProfilePicture() throws SQLException {
        ...
        try {
            ...
            if (chosenFile != null) {
                ...
                preparedStatement = connection.prepareStatement("SELECT picture FROM worker WHERE wid = ?");
                preparedStatement.setInt(1, currentUserId);
                resultSet = preparedStatement.executeQuery();
    ...
    public void updateProfile(ActionEvent actionEvent) throws IOException, SQLException {
        ...
        if (fullname.equals("") || email.equals("") || phone.equals("")) {
            ...
            try {
                ...
                psInsertValue = connection.prepareStatement("UPDATE worker SET fullname = ?, email = ?, phone = ? WHERE wid = ?");
                psInsertValue.setString(1, fullname);
                psInsertValue.setString(2, email);
                psInsertValue.setString(3, phone);
                psInsertValue.setString(4, String.valueOf(currentUserId));
                psInsertValue.executeUpdate();
